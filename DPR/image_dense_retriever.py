...
def load_images(dir = load_images_path, dummy=False):
    visual_feats = {}
    visual_pos = {}
...
# all_visual_feats, all_visual_pos = load_images(dummy = False)        
...
#             bath_img_id = img_id[batch_start : batch_start + bsz]
#             if query_token:
...
#             visual_feats = []
#             visual_pos = []
#             for ids in bath_img_id:
# #                 try:

#                 visual_feats.extend([all_visual_feats[ids]])
#                 visual_pos.extend([all_visual_pos[ids]])

# #                 except:
# #                 visual_feats.extend([torch.rand([1,36,2048])])
# #                 visual_pos.extend([torch.rand([1,36,4])])

#             visual_feats = torch.cat(visual_feats, dim=0).cuda()
#             visual_pos = torch.cat(visual_pos, dim=0).cuda()
            
#             if selector:
#                 rep_positions = selector.get_positions(q_ids_batch, tensorizer)

#                 _, out, _ = BiEncoder.get_visual_representation(
#                     question_encoder,
#                     q_ids_batch,
#                     q_seg_batch,
#                     q_attn_mask,
